...
public class LoginCheckInterceptor implements HandlerInterceptor {
	...
	public boolean preHandle(HttpServletRequest request,
		...
		if (sessionObj == null && !isOpenRequest(request)) {
			response.setContentType("text/html;charset=utf-8");
			String loginUrl = request.getContextPath() + "/common/toLogin";
			String content = "<script type='text/javascript'>var loginUrl='"
					+ loginUrl
					+ "';if(window != top){top.location.href=loginUrl;}else{location.href=loginUrl}</script>";
	...
	public boolean isOpenRequest(HttpServletRequest request) {
		...
		if (validUrl.startsWith("/common/login")
				|| validUrl.startsWith("/common/toLogin")
				|| validUrl.startsWith("/common/logout")) {
			return true;
		}
